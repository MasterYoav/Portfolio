...
- **React 19 islands** — command palette, topic filter, project dock
...
- **Color:** blue-black surfaces with a chartreuse signal in dark mode (the default); true off-white with an olive signal in light mode. The writing pages sit on a static blue–orchid gradient (`.blog-bg` in `global.css`), pure CSS so it costs nothing to render.
...
├── layouts/Base.astro        head, header, footer, command palette, optional blog gradient
├── components/
